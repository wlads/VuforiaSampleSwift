...
extension ViewController: VuforiaEAGLViewSceneSource, VuforiaEAGLViewDelegate {
    ...
    func scene(for view: VuforiaEAGLView!, userInfo: [String : Any]?) -> SCNScene! {
        ...
        if let sceneName = userInfo["scene"] as? String , sceneName == "stones" {
            ...
            if modelTag == 1 {
                ...
                return createCadeira(with: view)
            } else { // modelTag == 3
                return createMesa(with: view)
            }
        }else {
            print("chips scene")
    ...
    fileprivate func createChipsScene(with view: VuforiaEAGLView) -> SCNScene {
        ...
        // WB: default object scale?
        view.objectScale = 0.031750001013279
...
//        sceneView.backgroundColor = UIColor.white
    ...
    func createCadeira(with view: VuforiaEAGLView) -> SCNScene {
        // Load the .OBJ file
        guard let url = Bundle.main.url(forResource: "CADEIRA_URBAN_TRAMA_MARROM_B", withExtension: "obj") else {
            fatalError("Failed to find model file.")
        }
        let asset = MDLAsset(url:url)
        guard let object = asset.object(at: 0) as? MDLMesh else {
    ...
    func createMesa(with view: VuforiaEAGLView) -> SCNScene {
        // Load the .OBJ file
        guard let url = Bundle.main.url(forResource: "MESA_JANTAR_NEW_FOREST_P_CANYON_B", withExtension: "obj") else {
            fatalError("Failed to find model file.")
        }
        let asset = MDLAsset(url:url)
        guard let object = asset.object(at: 0) as? MDLMesh else {
